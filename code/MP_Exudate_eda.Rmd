...
# Mature DOC vs. NPP brokem down by season and incubation data

```{r}

doc_NPP_mature_date <- MP_DOC_NPP_phys_env_sum %>% 
  filter() %>% 
  filter(flag != 2) %>% 
  filter(age_d <= 50) %>% 
  ggplot(aes(x = NPP_umolC_g_hr, y = DOC_umolC_g_hr)) +
  ...
  ylab(expression(DOC~Production~(µmolC ~ g[DW]^{"-1"} ~ hr^{"-1"}))) +
  xlab(expression(NPP~(µmolC ~ g[DW]^{"-1"} ~ hr^{"-1"}))) +
  labs(color = "Incubation Date") + 
   geom_abline(slope = 0.023, intercept = 0.82, size = 1) +
...
# Linear Regression for each individual date

```{r}

MP_DOC_NPP_phys_env_sum_no_flags <- MP_DOC_NPP_phys_env_sum %>% 
  filter(flag != 2)

lm_030523_DOC_NPP <- lmodel2(MP_DOC_NPP_phys_env_sum$DOC_umolC_g_hr[MP_DOC_NPP_phys_env_sum$date == "5/3/23"] ~ MP_DOC_NPP_phys_env_sum$NPP_umolC_g_hr[MP_DOC_NPP_phys_env_sum$date == "5/3/23"])

lm_030523_DOC_NPP

lm_080923_DOC_NPP <- lmodel2(MP_DOC_NPP_phys_env_sum$DOC_umolC_g_hr[MP_DOC_NPP_phys_env_sum$date == "8/9/23"] ~ MP_DOC_NPP_phys_env_sum$NPP_umolC_g_hr[MP_DOC_NPP_phys_env_sum$date == "8/9/23"])

lm_080923_DOC_NPP

lm_082323_DOC_NPP <- lmodel2(MP_DOC_NPP_phys_env_sum_no_flags$DOC_umolC_g_hr[MP_DOC_NPP_phys_env_sum_no_flags$date == "8/23/23"] ~ MP_DOC_NPP_phys_env_sum_no_flags$NPP_umolC_g_hr[MP_DOC_NPP_phys_env_sum_no_flags$date == "8/23/23"])

lm_082323_DOC_NPP

lm_090523_DOC_NPP <- lmodel2(MP_DOC_NPP_phys_env_sum$DOC_umolC_g_hr[MP_DOC_NPP_phys_env_sum$date == "9/5/23"] ~ MP_DOC_NPP_phys_env_sum$NPP_umolC_g_hr[MP_DOC_NPP_phys_env_sum$date == "9/5/23"])

lm_090523_DOC_NPP

lm_121923_DOC_NPP <- lmodel2(MP_DOC_NPP_phys_env_sum$DOC_umolC_g_hr[MP_DOC_NPP_phys_env_sum$date == "12/19/23"] ~ MP_DOC_NPP_phys_env_sum$NPP_umolC_g_hr[MP_DOC_NPP_phys_env_sum$date == "12/19/23"])

lm_121923_DOC_NPP

lm_021124_DOC_NPP <- lmodel2(MP_DOC_NPP_phys_env_sum$DOC_umolC_g_hr[MP_DOC_NPP_phys_env_sum$date == "2/11/24"] ~ MP_DOC_NPP_phys_env_sum$NPP_umolC_g_hr[MP_DOC_NPP_phys_env_sum$date == "2/11/24"])

lm_021124_DOC_NPP

lm_041724_DOC_NPP <- lmodel2(MP_DOC_NPP_phys_env_sum$DOC_umolC_g_hr[MP_DOC_NPP_phys_env_sum$date == "4/17/24"] ~ MP_DOC_NPP_phys_env_sum$NPP_umolC_g_hr[MP_DOC_NPP_phys_env_sum$date == "4/17/24"])

lm_041724_DOC_NPP

lm_050724_DOC_NPP <- lmodel2(MP_DOC_NPP_phys_env_sum$DOC_umolC_g_hr[MP_DOC_NPP_phys_env_sum$date == "5/7/24"] ~ MP_DOC_NPP_phys_env_sum$NPP_umolC_g_hr[MP_DOC_NPP_phys_env_sum$date == "5/7/24"])
...
matureDOC.ftest <- var.test( DOC~ season, data = mature_DOC)

matureDOC.ftest
# DOC Release Rates Mature Kelp 

res_DOC_mature_season <- t.test(mature_DOC$DOC[mature_DOC$season == "Summer"],mature_DOC$DOC[mature_DOC$season == "Spring"], var.equal = FALSE)
...
summerDOC.ftest <- var.test(DOC~ phys_state, data = summer_DOC)
...
springDOC.ftest <- var.test( DOC~ phys_state, data = spring_DOC)
# DOC Release Rates Mature Kelp 
...
respiration <- MP_DOC_NPP_phys_env_sum %>% 
  filter(season %in% c("Spring", "Summer")) %>% 
  filter(flag != 2) %>% 
  filter(PAR == 0) %>% 
  select(age_d, season, NPP_umolC_g_hr) %>% 
  summarize(mean_resp = mean(NPP_umolC_g_hr))
...
lm_PER_CN <- lmodel2(PER ~ C_N, data = PER_C_N)

lm_PER_CN_model1 <- lm(PER ~ C_N, data = PER_C_N)

lm_PER_CN

summary(lm_PER_CN_model1)

PER_CN10 <- 10*-0.0468 + 3.4496
PER_CN40 <- 40*-0.0468 + 3.4496


PER_C_N_plot <-  MP_DOC_NPP_phys_env_sum %>% 
  filter(flag != 2, season %in% c("Spring", "Summer")) %>% 
  select(age_d, season, PAR,DOC_umolC_g_hr,NPP_umolC_g_hr, C_N, Chl_C, chlA_nmol_g) %>% 
  filter(PAR > 0) %>% 
  filter(age_d < 50) %>% 
  ...
  geom_abline(intercept = 2.49, slope = -0.047, size = 2) +
  geom_point(size = 4) +
  labs(y = "Percent Extracellular Release",
       x = "Tissue C:N") + 
  theme(panel.background = element_rect(fill = "white", color = "black", size = 1.5),
...
summary(CN_physiostate_season.aov3)
...
mature_seasonal_ChlC_welchs_test <- t.test(Chl_C ~ season, data = mature_ChlC,var.equal = FALSE, alternative = "greater")

mature_seasonal_ChlC_welchs_test # Spring time CN is significantly lower than Summer time CN for mature kelp
...
DOC_ManURA_lm = lmodel2(data = DOC_Sugar, mean_DOC ~ `Man-URA`)

DOC_ManURA_lm# R2 = 0.50, p = 9.73e-08
